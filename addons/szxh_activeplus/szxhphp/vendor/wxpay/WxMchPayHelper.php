...
//        }
    ...
    private function getPem() {
        ...
        $szxhCachePath = ATTACHMENT_ROOT . 'szxh_cache';
        if(!is_dir($szxhCachePath)) {
            mkdir($szxhCachePath);
        }
        
        $certPath = $szxhCachePath . '/szxh_cert_uniacid'.$_W['uniacid'].'.pem';
        $keyPath  = $szxhCachePath . '/szxh_key_uniacid'.$_W['uniacid'].'.pem';
