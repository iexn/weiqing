...
namespace szxh\template;

include MFRAME. '/library/template/TagLib.php';
include MFRAME. '/library/template/driver/File.php';
include MFRAME. '/library/template/taglib/Cx.php';
...
class Template
{
    ...
    // 引擎配置
    protected $config = [
        'view_path'          => MROOT . '/szxh/view/', // 模板路径
        'view_base'          => '',
        'view_suffix'        => 'html', // 默认模板文件后缀
        'view_depr'          => DS,
        'cache_suffix'       => 'html', // 默认模板缓存后缀
    ...
    {
        $this->config['cache_path'] = MROOT.'/template/';
        ...
        $class         = false !== strpos($type, '\\') ? $type : '\\szxh\\template\\driver\\' . ucwords($type);
    ...
    public function fetch($template, $vars = [], $config = [])
    {
        ...
        if ($template) {
            ...
            $cacheFile = $this->config['cache_path'] . $orTemplate . '.' . ltrim($this->config['cache_suffix'], '.');;
    ...
    public function parseTagLib($tagLib, &$content, $hide = false)
    {
        if (false !== strpos($tagLib, '\\')) {
            ...
            $className = '\\szxh\\template\\taglib\\' . ucwords($tagLib);
        }
        /** @var Taglib $tLib */
    ...
    private function parseTemplateFile($template)
    {
        if ('' == pathinfo($template, PATHINFO_EXTENSION)) {
            ...
            $P = $path . $template . '.' . ltrim($this->config['view_suffix'], '.');
            $template = realpath($P);
        }
        ...
        if (is_file($template)) {
            ...
            throw new \Exception('template not exists:' . $P, $template);
        }
    }
